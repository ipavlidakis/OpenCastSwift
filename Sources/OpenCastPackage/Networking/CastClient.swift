...
public final class CastClient: NSObject, RequestDispatchable, Channelable {
    ...
    fileprivate func sendConnectMessage() throws {
        ...
        DispatchQueue.main.async {
            self.add(channel: self.heartbeatChannel)
            self.add(channel: self.connectionChannel)
            self.add(channel: self.receiverControlChannel)
            self.add(channel: self.mediaControlChannel)
            
            if self.device.capabilities.contains(.multizoneGroup) {
                self.add(channel: self.multizoneControlChannel)
            }
        }
    }
